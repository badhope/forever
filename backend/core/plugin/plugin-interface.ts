...
export interface MemoryInsight {
  type: 'recent' | 'important' | 'emotional' | 'pattern';
  label: string;
  memories?: Memory[];
...
export interface MemoryPattern {
  type: 'emotion_distribution' | 'importance_stats' | 'recency_pattern' | 'topic_clusters';
  label: string;
  data: Record<string, any>;
}
...
export interface MemoryPlugin extends Plugin {
   ...
   * @param query 可选查询条件
   */
  reflect?(characterId: string, query?: string): Promise<MemoryReflection>;
}
