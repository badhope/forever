...
import { chat, type ChatMessage } from '../llm/index';
...
6. 只返回 JSON，不要其他内容`,
   ...
   */
  private decomposeByKeywords(goal: string, _planId: string): Task[] {
    const tasks: Task[] = [];
    ...
    const actionKeywords: Array<{
      ...
      {
        keywords: ['分析', '研究', 'analyze', 'research'],
